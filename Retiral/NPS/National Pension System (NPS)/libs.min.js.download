...
// source: http://code.jquery.com/jquery-1.11.3.min.js
...
// this source code has been minified with google closure compiler at http://closure-compiler.appspot.com/home
// from https://raw.githubusercontent.com/jquery-i18n-properties/jquery-i18n-properties/master/jquery.i18n.properties.js
(function(m){function p(c,a){m.ajax({url:c,async:!1,cache:a.cache,contentType:"text/plain;charset="+a.encoding,dataType:"text",success:function(c,b){t(c,a.mode)}})}function t(c,a){for(var d="",b=c.split(/\n/),g=/(\{\d+\})/g,f=/\{(\d+)\}/g,u=/(\\u.{4})/ig,e=0;e<b.length;e++)if(b[e]=b[e].replace(/^\s\s*/,"").replace(/\s\s*$/,""),0<b[e].length&&"#"!=b[e].match("^#")){var l=b[e].split("=");if(0<l.length){for(var q=unescape(l[0]).replace(/^\s\s*/,"").replace(/\s\s*$/,""),h=1==l.length?"":l[1];"\\"==h.match(/\\$/);)h= h.substring(0,h.length-1),h+=b[++e].replace(/\s\s*$/,"");for(var n=2;n<l.length;n++)h+="="+l[n];h=h.replace(/^\s\s*/,"").replace(/\s\s*$/,"");if("map"==a||"both"==a){if(l=h.match(u))for(n=0;n<l.length;n++)h=h.replace(l[n],v(l[n]));m.i18n.map[q]=h}if("vars"==a||"both"==a)if(h=h.replace(/"/g,'\\"'),w(q),g.test(h)){for(var l=h.split(g),n=!0,k="",p=[],r=0;r<l.length;r++)!g.test(l[r])||0!=p.length&&-1!=p.indexOf(l[r])||(n||(k+=","),k+=l[r].replace(f,"v$1"),p.push(l[r]),n=!1);d+=q+"=function("+k+"){";q= '"'+h.replace(f,'"+v$1+"')+'"';d+="return "+q+";};"}else d+=q+'="'+h+'";'}}eval(d)}function w(c){if(/\./.test(c)){var a="";c=c.split(/\./);for(var d=0;d<c.length;d++)0<d&&(a+="."),a+=c[d],eval("typeof "+a+' == "undefined"')&&eval(a+"={};")}}function x(c){return c&&c.constructor==Array?c:[c]}function v(c){var a=[];c=parseInt(c.substr(2),16);0<=c&&c<Math.pow(2,16)&&a.push(c);c="";for(var d=0;d<a.length;++d)c+=String.fromCharCode(a[d]);return c}m.i18n={};m.i18n.map={};m.i18n.properties=function(c){c= m.extend({name:"Messages",language:"",path:"",mode:"vars",cache:!1,encoding:"UTF-8",callback:null},c);if(null===c.language||""==c.language)c.language=m.i18n.browserLang();null===c.language&&(c.language="");var a=[];m.ajax({url:c.path+"languages.json",async:!1,cache:!1,success:function(b,c){a=b.languages}});for(var d=x(c.name),b=0;b<d.length;b++){p(c.path+d[b]+".properties",c);if(2<=c.language.length){var g=c.language.substring(0,2);0<a.length&&-1!=m.inArray(g,a)&&p(c.path+d[b]+"_"+g+".properties", c)}5<=c.language.length&&(g=c.language.substring(0,5),0<a.length&&-1!=m.inArray(g,a)&&p(c.path+d[b]+"_"+g+".properties",c))}c.callback&&c.callback()};m.i18n.prop=function(c){var a=m.i18n.map[c];if(null==a)return"["+c+"]";var d;2==arguments.length&&m.isArray(arguments[1])&&(d=arguments[1]);var b;if("string"==typeof a){for(b=0;-1!=(b=a.indexOf("\\",b));)a="t"==a.charAt(b+1)?a.substring(0,b)+"\t"+a.substring(b++ +2):"r"==a.charAt(b+1)?a.substring(0,b)+"\r"+a.substring(b++ +2):"n"==a.charAt(b+1)?a.substring(0, b)+"\n"+a.substring(b++ +2):"f"==a.charAt(b+1)?a.substring(0,b)+"\f"+a.substring(b++ +2):"\\"==a.charAt(b+1)?a.substring(0,b)+"\\"+a.substring(b++ +2):a.substring(0,b)+a.substring(b+1);var g=[],f,k;for(b=0;b<a.length;)if("'"==a.charAt(b))if(b==a.length-1)a=a.substring(0,b);else if("'"==a.charAt(b+1))a=a.substring(0,b)+a.substring(++b);else{for(f=b+2;-1!=(f=a.indexOf("'",f));)if(f==a.length-1||"'"!=a.charAt(f+1)){a=a.substring(0,b)+a.substring(b+1,f)+a.substring(f+1);b=f-1;break}else a=a.substring(0, f)+a.substring(++f);-1==f&&(a=a.substring(0,b)+a.substring(b+1))}else if("{"==a.charAt(b))if(f=a.indexOf("}",b+1),-1==f)b++;else if(k=parseInt(a.substring(b+1,f)),!isNaN(k)&&0<=k){var e=a.substring(0,b);""!=e&&g.push(e);g.push(k);b=0;a=a.substring(f+1)}else b=f+1;else b++;""!=a&&g.push(a);a=g;m.i18n.map[c]=g}if(0==a.length)return"";if(1==a.lengh&&"string"==typeof a[0])return a[0];e="";for(b=0;b<a.length;b++)e="string"==typeof a[b]?e+a[b]:d&&a[b]<d.length?e+d[a[b]]:!d&&a[b]+1<arguments.length?e+arguments[a[b]+ 1]:e+("{"+a[b]+"}");return e};m.i18n.browserLang=function(){var c=navigator.language||navigator.userLanguage,c=c.toLowerCase();3<c.length&&(c=c.substring(0,3)+c.substring(3).toUpperCase());return c};var k;k||(k=function(c,a,d){if("[object RegExp]"!==Object.prototype.toString.call(a))return"undefined"==typeof k._nativeSplit?c.split(a,d):k._nativeSplit.call(c,a,d);var b=[],g=0,f=(a.ignoreCase?"i":"")+(a.multiline?"m":"")+(a.sticky?"y":"");a=RegExp(a.source,f+"g");var m,e,l;c+="";k._compliantExecNpcg|| (m=RegExp("^"+a.source+"$(?!\\s)",f));if(void 0===d||0>+d)d=Infinity;else if(d=Math.floor(+d),!d)return[];for(;e=a.exec(c);){f=e.index+e[0].length;if(f>g&&(b.push(c.slice(g,e.index)),!k._compliantExecNpcg&&1<e.length&&e[0].replace(m,function(){for(var a=1;a<arguments.length-2;a++)void 0===arguments[a]&&(e[a]=void 0)}),1<e.length&&e.index<c.length&&Array.prototype.push.apply(b,e.slice(1)),l=e[0].length,g=f,b.length>=d))break;a.lastIndex===e.index&&a.lastIndex++}g===c.length?!l&&a.test("")||b.push(""): b.push(c.slice(g));return b.length>d?b.slice(0,d):b},k._compliantExecNpcg=void 0===/()??/.exec("")[1],k._nativeSplit=String.prototype.split);String.prototype.split=function(c,a){return k(this,c,a)}})(jQuery);
/*!
...
// minfied source: https://raw.githubusercontent.com/furf/jquery-ui-touch-punch/master/jquery.ui.touch-punch.min.js
...
// source: http://momentjs.com/downloads/moment-with-locales.js
...
// source: http://underscorejs.org/underscore-min.js
...
/**
 * Bosch Connect Widget Utility functions
 * 
 * 
 * Documentation: TODO
 *
...
(function(window, $, undefined) {
	...
	var LOG_ID = "com.bosch.connect.widgetUtils";

	/**
	 * Returns true if the current user is a community owner, otherwise false.
	 */
	var isCommunityOwner = function( iContext ) {
		
		debug(LOG_ID, '<<<<<<< isCommunityOwner in widgetUtils >>>>>>');
		if(iContext === undefined || iContext == null) return;
		
		var canPersonalize = iContext.getUserProfile().getItemValue('canPersonalize');
		debug(LOG_ID, 'widgetUtils canPersonalize=' + canPersonalize);

		// if current user is communnity owner, build up the Menu
		if (canPersonalize == 'true') {
			return true;
		} else {
			return false;
		}
	...
	/**
	 * Returns the lang of the user passed via the widget-config.xml
	 */
	var getUserLanguage = function (){
		debug(LOG_ID, ' getUserLanguage >>>>>>');
		// 'currentBrowserLocale' is set in header.jsp as a hidden field , the value is retrieved from  server side as we couldn't rely on java script to fetch the lang details
		 var browserLanguage = $("#currentBrowserLocale").text();
		if(browserLanguage === undefined || browserLanguage == null) return 'en';
		
		debug(LOG_ID, ' getUserLanguage <<<<<<<'+browserLanguage);
		return browserLanguage ;
	};
	
	/**
	 * Format date based on browser locale in the same format as it is used in IBM Connections Widget.
	 * 
	 * Note: All timestamps returned from the SP API are UTC
	 * 
	 * For English (Default): Nov 12, 2014
	 * 
	 * For German: 12.05.2015
	 * 
	 * We use moment.js to format the date.
	 * 
	 * @param date
	 *            A date as a string
	 * @returns {String} A formated date
	 */
	function formatDate(date,userLanguage) {

		var defaultLanguage = 'en';
		userLanguage = (userLanguage === 'en' || userLanguage === 'de') ? userLanguage : defaultLanguage;

		//debug(LOG_ID, 'date=' + date + ', viewMode.lang=' + viewMode.lang + ', userLanguage=' + userLanguage);

		var formattedDate = '';

		var formattedMoment = moment(date, moment.ISO_8601);

		try {
			if (userLanguage === 'de') {
				formattedMoment.locale('de');
			} else {
				formattedMoment.locale('en');
			}

			// Use the localized format of moment.js
			formattedDate = formattedMoment.format('L');

			debug(LOG_ID, 'formattedDate=' + formattedDate);

		} catch (err) {
			error(LOG_ID, err);
			error(LOG_ID, 'Something went wrong. We use the original date from the API');
			formattedDate = date;
		}
	...
	/**
	 * Format time based on browser locale in the same format as it is used in IBM Connections Widget.
	 * 
	 * Note: All timestamps returned from the SP API are UTC
	 * 
	 * For English (Default): 9:36 AM
	 * 
	 * For German: 09:36
	 * 
	 ...
	 * 
	 * @param date
	 * @returns {String} A formated time
	 */
	function formatTime(date,userLanguage) {
		...
		try {
			...
			// Use the localized format of moment.js
			formattedTime = formattedMoment.format('LT');

			debug(LOG_ID, 'formattedTime=' + formattedTime);
	...
	/******* store2.js related functions ********/
	
	/**
	 * Checks whether the localStorage or sessionStorage has already the namespace 
	 * return the namespace local storage if exists else create a new one and returns
	 * 
	 * @param namespacevalue
	 */
	function getLocalStoreageWithNameSpace( namespacevalue, sessionStorage ){
		debug(LOG_ID, 'namespacevalue=' + namespacevalue + ' / sessionStorage=' + sessionStorage);
		if(store[namespacevalue] === undefined || store[namespacevalue].size() === 0){
			debug(LOG_ID, 'Creating a new namespace for ' + namespacevalue);
			if (sessionStorage){
				debug(LOG_ID, 'using session storage');
				return store.session.namespace(namespacevalue);
			} else {
				debug(LOG_ID, 'using local storage');
				return  store.namespace(namespacevalue);	
			}
		}
		
		if (sessionStorage){
			debug(LOG_ID, 'using session storage');
			return  store.session[namespacevalue];
		} else {
			debug(LOG_ID, 'using local storage');
			return  store[namespacevalue];			
		}
		
	}
	/**
	 * Stores the given key and value in localStorage or sessionStorage under the namespace object using the library store2.js
	 * 
	 * @param namespace
	 * @param key 
	 * @parm value
	 * @refers https://github.com/nbubna/store
	 */
	function storeInLocalStorageWithNamespace( namespacevalue , key , value, sessionStorage ) {
		    var localStorageNameSpace = getLocalStoreageWithNameSpace( namespacevalue, sessionStorage );
			localStorageNameSpace.set(key,value);
	}
	
	
	/**
	 * Stores the given key and value in localStorage or sessionStorage using the library store2.js
	 * 
	 * @param key 
	 * @parm value
	 * @refers https://github.com/nbubna/store
	...
	/**
	 * Checks whether the given key is available in localStorage or sessionStorage
	 * @param key
	 */
	function isKeyAvailableInLocalStorageWithNamespace( namespacevalue , key, sessionStorage ){
		var localStorageNameSpace = getLocalStoreageWithNameSpace( namespacevalue, sessionStorage );
		return localStorageNameSpace.has(key);
	}
	
	/**
	 * Checks whether the given is available in local storage
	 * @param key
	 */
	function isKeyAvailableInLocalStorage(key, sessionStorage){
		if (sessionStorage){
	...
	/**
	 * Checks whether the given key exists in localStorage or sessionStorage and retrieves the value if exists
	 * @param key
	 */
	function checkAndGetValueFromLocalStorageWithNameSpace( namespacevalue , key, sessionStorage ){
		if(isKeyAvailableInLocalStorageWithNamespace(namespacevalue , key, sessionStorage)){
			var localStorageNameSpace = getLocalStoreageWithNameSpace( namespacevalue , sessionStorage);
			return localStorageNameSpace.get(key);
		}
	 ...
	 * @param key
	 */
	function checkAndGetValueFromLocalStorage(key, sessionStorage){
		if(isKeyAvailableInLocalStorage(key, sessionStorage)){
			if (sessionStorage){
	 ...
	 */
	function clearLocalStorageWithNamespace( namespacevalue, sessionStorage ){
		var localStorageNameSpace = getLocalStoreageWithNameSpace( namespacevalue, sessionStorage );
		localStorageNameSpace.clearAll();
	}
	...
	/**
	 * checks whether jquery-ui dialog has already been initialized on the given dom element
	 * 	 * @param jquery dom element
	 */
	var isJqeuryUIDialogInitialized = function( $domElement ) {
		return $domElement.hasClass("ui-dialog-content");
	};
	/**
	 * Opens the given url in new window
	 * @param url
	 */
	var openBlankWindow = function( url ) {
		window.open(url, '_blank');
	};
	
	widgetUtils.isCommunityOwner = isCommunityOwner;
	widgetUtils.getUserLanguage = getUserLanguage;
	widgetUtils.openBlankWindow = openBlankWindow;
	widgetUtils.formatDate = formatDate;
	widgetUtils.formatTime = formatTime;
	widgetUtils.storeInLocalStorage = storeInLocalStorage;
	widgetUtils.isKeyAvailableInLocalStorage = isKeyAvailableInLocalStorage;
	widgetUtils.checkAndGetValueFromLocalStorage = checkAndGetValueFromLocalStorage;
	widgetUtils.clearLocalStorage = clearLocalStorage;
	widgetUtils.getLocalStoreageWithNameSpace = getLocalStoreageWithNameSpace;
	widgetUtils.storeInLocalStorageWithNamespace = storeInLocalStorageWithNamespace;
	widgetUtils.isKeyAvailableInLocalStorageWithNamespace = isKeyAvailableInLocalStorageWithNamespace;
	widgetUtils.checkAndGetValueFromLocalStorageWithNameSpace = checkAndGetValueFromLocalStorageWithNameSpace;
	widgetUtils.clearLocalStorageWithNamespace = clearLocalStorageWithNamespace;
	widgetUtils.isJqeuryUIDialogInitialized = isJqeuryUIDialogInitialized;
